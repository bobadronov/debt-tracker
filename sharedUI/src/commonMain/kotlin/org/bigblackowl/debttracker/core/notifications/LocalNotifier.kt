 ...
 * `UNUserNotificationCenter`, Desktop System Tray, Web `Notification` API) — a plain interface
 * (not `expect class`) so [org.bigblackowl.debttracker.preview.previewModule] can bind a no-op
 * implementation instead of the real one, and Compose Preview never touches platform notification APIs.
 */
...
// No `expect fun createLocalNotifier()` — the Android implementation needs a
